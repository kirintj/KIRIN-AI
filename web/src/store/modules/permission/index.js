  ...
  return routes.map((e) => {
    ...
    if (e.children && e.children.length > 0) {
      route.children = e.children.map((e_child) => {
        let componentPath = `/src/views${e_child.component}/index.vue`;
        if (e_child.component === '/chat/chathistory') {
          componentPath = `/src/views/system/chathistory/index.vue`;
        }
        if (e_child.component === '/chat/chat') {
          componentPath = `/src/views/chat/index.vue`;
        }
        return {
          name: e_child.name,
          path: e_child.path,
